      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {filteredOrders.map((order) => (
            <div key={order.orderId} className="bg-white p-3 sm:p-6 rounded-lg shadow-md">
              {/* Order Header */}
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 w-full">
                  ...
                  <div className="relative ship-to-container group">
                    ...
                    <button
                      ...
                      onClick={() => setShowShipTo(showShipTo === order.orderId ? null : order.orderId)}
                    >
                      {order.address.fullName}
                      <FaChevronDown className={`w-3 h-3 transform transition-transform ${showShipTo === order.orderId ? 'rotate-180' : ''}`} />
                    </button>
                    
                    {/* Ship To Dropdown */}
                    <div className="absolute top-full left-0 mt-1 bg-white border rounded-lg shadow-lg p-4 z-10 min-w-[250px] opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 ease-in-out"
                         style={{ visibility: showShipTo === order.orderId ? 'visible' : '', opacity: showShipTo === order.orderId ? '1' : '' }}>
                      ...
                      <p className="text-sm text-gray-600">{order.address.flatHouseNo}, {order.address.apartment}</p>
                      <p className="text-sm text-gray-600">{order.address.areaStreet}</p>
              ...
              <div className="mt-4 flex flex-col sm:flex-row gap-4">
                ...
                <div className="flex-1 flex flex-col justify-between">
                  <div>
                    ...
                    <p className="text-sm text-gray-600">{formatSpecifications(order.specifications)}</p>
                  </div>
