...
const Cart = () => {
  ...
  useEffect(() => {
    ...
    const calculateTotalPrice = () => {
      ...
      const newSubtotal = cart.reduce(
        (total, cartItem) => total + cartItem.totalPartsPrice,
        0
      );
  ...
  useEffect(() => {
    const fetchCompareItems = async () => {
      try {
        const updatedCompareItems = await compareAPI.getAllComparisons(userId);
        setCompareItems(updatedCompareItems);

        // Create a mapping of cart items that are already in comparison
        const initialComparisonStatus = {};
        cart.forEach((cartItem) => {
          const isInComparison = updatedCompareItems.some(
            (compareItem) => compareItem.cartId === cartItem.cartId
          );
          initialComparisonStatus[cartItem.cartId] = isInComparison;
        });
        setComparisonStatus(initialComparisonStatus);
      } catch (error) {
  ...
  const handleAddToCompare = async (cartItem) => {
    try {
      // Check if already at max comparison limit
      if (compareItems.length >= 4) {
        toast.error("You can compare a maximum of 4 items");
        return;
      }

      // Check if item is already in comparison
      const isAlreadyInComparison = compareItems.some(
        (item) => item.cartId === cartItem.cartId
      );

      if (isAlreadyInComparison) {
        toast.error("This item is already in comparison");
        return;
      }

      // Add to comparison
      const response = await compareAPI.addToCompare(cartItem.cartId, userId);

      // Fetch updated comparison items
      const updatedCompareItems = await compareAPI.getAllComparisons(userId);
      setCompareItems(updatedCompareItems);

      // Update comparison status for this cart item
      setComparisonStatus((prev) => ({
        ...prev,
        [cartItem.cartId]: true,
      }));
  ...
  const handleViewCompare = async () => {
    try {
      // Ensure at least 2 items are in comparison
      ...
      toast.error("Failed to fetch comparison items");
    }
  };
  return (
    <>
                        ...
                        <div className="flex flex-col justify-between ml-3 sm:ml-4 flex-grow">
                          ...
                          <div className="flex flex-col lg:flex-row items-start lg:items-center gap-2 lg:gap-4 mt-2">
                            <button
                              ...
                              onClick={() =>
                                handleRemoveOneCart(cartItem.cartId)
                              }
                            >
                            ...
                            <button
                              ...
                              disabled={
                                comparisonStatus[cartItem.cartId] ||
